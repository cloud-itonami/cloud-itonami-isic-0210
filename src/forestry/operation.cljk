(ns forestry.operation
  "ForestryOperationActor -- one silviculture operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (ForestryAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Forest Coordination Governor (:govern) and the rollout phase gate
  (:decide) before anything commits to the SSoT.

  One graph run = one forestry operation (assess -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A stand's lifecycle is advanced by
  MANY ops (assessment / scheduling / monitoring), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human forester/operations manager. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [forestry.advisor :as advisor]
...
(defn build
  "Compiles a ForestryOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `forestry.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ForestryAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Forest Coordination Governor -- independent censor.
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :requires-forester-approval
